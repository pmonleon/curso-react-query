...
export const IssueView = () => {
  ...
  return (
    <div className="row mb-5">
      ...
      {/* Comentario de otros */}
      {
        issueCommentsQuery.isLoading && !issueQuery.isLoading && <div style={{marginTop: '30px', display:'flex', justifyContent:'center'}}> 
          <LoadingIcon />
        </div> 
      }
      {
        !!issueCommentsQuery?.data?.length && issueCommentsQuery.data?.map(item => (
          <IssueComment key={item.id}  issue ={ item } />
        ))
      }
